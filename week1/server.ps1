...
function Send-FileResponse($response, $filePath, $contentType) {
    if (Test-Path $filePath) {
        ...
        $nb = [System.Text.Encoding]::UTF8.GetBytes("404 Not Found: $filePath")
...
while ($listener.IsListening) {
    try {
        ...
        if ($path -eq "/api/history" -and $method -eq "GET") {
            ...
            $contentType = switch -Wildcard ($fullPath) {
            ...
            Send-FileResponse -response $response -filePath $fullPath -contentType $contentType
        }
    } catch {
